...
git init
git add .
git commit -m "Scoring résiliation : pipeline + app Streamlit"
git branch -M main
git remote add origin https://github.com/VOTRE_COMPTE/scoring-resiliation.git
git push -u origin main
```

Le mot de passe demandé est un **Personal Access Token** (GitHub → Settings → Developer settings → Tokens).

✅ **Vérifiez sur GitHub que `models/pipeline_resiliation.pkl` est bien présent**, sinon l'application en ligne plantera avec `FileNotFoundError`.

### 2. Déployer

1. https://share.streamlit.io, connexion avec GitHub
2. **Create app** → **Deploy a public app from GitHub**
3. Repository `VOTRE_COMPTE/scoring-resiliation` · Branch `main` · Main file path `app.py`
4. **Advanced settings** → version de Python identique à la vôtre (`python --version`)
5. **Deploy** — en ligne en 1 à 3 minutes

### 3. Mettre à jour

```bash
git add .
git commit -m "Ajustement du seuil de risque"
git push
```

Streamlit Cloud redéploie automatiquement à chaque push.

### Pourquoi pas Vercel ?

Une application Streamlit est un **processus Python permanent** doublé d'une connexion WebSocket. Vercel est conçu pour du statique et des fonctions serverless qui s'éteignent après chaque requête : les deux modèles sont incompatibles. Pour déployer sur Vercel, il faudrait remplacer Streamlit par une API FastAPI (`POST /predict`) et écrire la page web séparément.
...
|---|---|---|
| `ModuleNotFoundError: No module named 'sklearn'` | `requirements.txt` absent ou incomplet | Ajouter `scikit-learn`, puis Manage app → Reboot |
| `FileNotFoundError: models/pipeline_resiliation.pkl` | Le `.pkl` n'est pas dans le dépôt | Vérifier `.gitignore`, `git add models/`, push |
| `InconsistentVersionWarning` au `joblib.load` | Version de scikit-learn différente | Relancer `python train_model.py`, qui refige `requirements.txt` |
| Fonctionne en local, pas en ligne | Chemin absolu `C:/Users/...` | Chemins relatifs uniquement (`Path(__file__).parent`) |
| Échec d'installation de pandas / numpy | Version de Python du cloud inadaptée | Advanced settings → même version que la vôtre |

## Limites connues

- **500 clients, 50 résiliations** : l'échantillon est petit, les métriques de test reposent sur 10 positifs et restent donc instables.
- Le modèle donne les **importances globales** de la forêt, pas l'explication d'une prédiction individuelle. Pour cela, il faudrait ajouter SHAP.
- Aucune variable comportementale (réclamations, appels au service client, retards de paiement) — c'est là que se trouveraient les gains suivants.
# scoring-resiliation
